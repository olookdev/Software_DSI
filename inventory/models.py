...
class List_Stok(models.Model):
    ...
    jenis = models.ForeignKey(
        JenisBarang, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class HargaStok(models.Model):
    barang = models.ForeignKey(
        "List_Stok", on_delete=models.CASCADE, related_name="daftar_harga"
    )
    suplier = models.ForeignKey(
        "Suplier", on_delete=models.CASCADE, related_name="harga_suplier"
    )
...
class HargaJual(models.Model):
    ...
    bahan_baku = models.ManyToManyField(
        "List_Stok", through="HargaJualBahan", related_name="produk_jual"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.kode_produk:
            ...
            jumlah_produk_bulan_ini = HargaJual.objects.filter(
                kode_produk__startswith=prefix
            ).count()
    ...
    def total_modal(self):
        total_bahan = sum(
            item.harga_stok_terpilih.harga_satuan for item in self.list_bahan.all()
        )
...
class HargaJualBahan(models.Model):
    harga_jual = models.ForeignKey(
        HargaJual, on_delete=models.CASCADE, related_name="list_bahan"
    )
...
class ArusStok(models.Model):
    ...
    barang = models.ForeignKey(
        "List_Stok", on_delete=models.CASCADE, related_name="arus_stok"
    )
...
class OrderUtama(models.Model):
    ...
    no_order = models.CharField(
        max_length=50, unique=True, verbose_name="No Order", editable=False
    )
    ...
    nama_order = models.CharField(
        max_length=255, verbose_name="Nama Order / Judul Project"
    )
    total_harga = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00, verbose_name="Total Harga"
    )
    uang_muka = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00, verbose_name="Uang Muka (DP)"
    )
    sisa_bayar = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00, verbose_name="Sisa Pembayaran"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.no_order:
            ...
            tahun_bulan = sekarang.strftime("%Y%m")
            order_terakhir = (
                OrderUtama.objects.filter(no_order__contains=f"ORD-{tahun_bulan}-")
                .order_by("-no_order")
                .first()
            )
    ...
    def sisa_qty_per_item(self):
        ...
        for item in self.items.all():
            total_terkirim = (
                PengirimanDetail.objects.filter(order_detail=item).aggregate(
                    total=Sum("qty_kirim")
                )["total"]
                or 0
            )
...
class OrderDetail(models.Model):
    order_utama = models.ForeignKey(
        OrderUtama, on_delete=models.CASCADE, related_name="items"
    )
    nama_pesanan = models.CharField(
        max_length=255, help_text="Nama cetakan/job, misal: Banner Warung Pecel"
    )
    nama_item = models.CharField(
        max_length=255,
        help_text="Nama produk dari tabel harga_jual, misal: BANNER OUTDOOR",
    )
    kode_item = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        help_text="Kode barang/bahan, misal: 0500900001",
    )
    qty = models.IntegerField(default=1)
    panjang = models.DecimalField(
        max_digits=10, decimal_places=2, default=1.00, help_text="Dalam meter"
    )
    lebar = models.DecimalField(
        max_digits=10, decimal_places=2, default=1.00, help_text="Dalam meter"
    )
    harga_dasar = models.DecimalField(
        ...
        decimal_places=2,
        default=0.00,
        help_text="Harga modal/beli bahan",
    )
    harga_jual = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=0.00,
        help_text="Harga jual per meter/satuan",
    )
    ...
    keterangan = models.TextField(
        blank=True, null=True, help_text="Catatan finishing: mata ayam, lipat pres, dll"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class PiutangPelanggan(models.Model):
    order = models.OneToOneField(
        "OrderUtama", on_delete=models.CASCADE, related_name="piutang"
    )
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="Belum Lunas"
    )
    updated_at = models.DateTimeField(auto_now=True)
...
class CicilanPiutang(models.Model):
    piutang = models.ForeignKey(
        PiutangPelanggan, on_delete=models.CASCADE, related_name="list_cicilan"
    )
...
def otomatis_buat_piutang(sender, instance, created, **kwargs):
    if instance.sisa_bayar and float(instance.sisa_bayar) > 0:
        PiutangPelanggan.objects.update_or_create(
            order=instance,
            defaults={"sisa_piutang": instance.sisa_bayar, "status": "Belum Lunas"},
        )
    else:
...
class Transaksi(models.Model):
    ...
    tanggal = models.DateField(
        auto_now=False, auto_now_add=False, help_text="tanggal transaksi dibuat"
    )
    keterangan = models.TextField(
        blank=True, null=True, help_text="keterangan transaksi"
    )
    jenis = models.CharField(
        max_length=11,
        choices=Jenis.choices,
        help_text="jenis transaksi pemasukan atau pengeluaran",
    )
    nominal = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="nominal transaksi"
    )


class Hutang(models.Model):
    arus_stok = models.OneToOneField(
        "ArusStok", on_delete=models.CASCADE, related_name="hutang_detail"
    )
    ...
    def __str__(self):
        supplier_nama = (
            self.arus_stok.suplier.nama_suplier
            if self.arus_stok.suplier
            else "Tanpa Supplier"
        )
...
class StokOpname(models.Model):
    ...
    tanggal = models.DateTimeField(default=timezone.now)
    barang = models.ForeignKey(
        List_Stok, on_delete=models.SET_NULL, null=True, related_name="stok_opname"
    )
    jenis = models.CharField(max_length=10, choices=JENIS_CHOICES)
    qty_sistem = models.IntegerField(
        help_text="Jumlah stok di komputer saat opname dilakukan"
    )
    qty_gudang = models.IntegerField(
        help_text="Jumlah fisik barang yang dihitung nyata di gudang"
    )
    selisih = models.IntegerField(
        default=0, help_text="Hasil dari qty_gudang - qty_sistem"
    )
    stok_akhir = models.IntegerField(help_text="Stok final setelah disesuaikan")
    keterangan = models.TextField(
        blank=True, null=True, help_text="Catatan tambahan alasan selisih"
    )

    def __str__(self):
        nama_barang = self.barang.nama_barang if self.barang else "Barang Dihapus"
        return (
            f"Opname {nama_barang} ({self.tanggal.strftime('%d-%m-%Y')}) - {self.jenis}"
        )
...
class Pengiriman(models.Model):
    order = models.ForeignKey(
        OrderUtama, on_delete=models.CASCADE, related_name="pengiriman"
    )
...
class PengirimanDetail(models.Model):
    pengiriman = models.ForeignKey(
        Pengiriman, on_delete=models.CASCADE, related_name="detail_items"
    )
    order_detail = models.ForeignKey(
        OrderDetail, on_delete=models.CASCADE, related_name="riwayat_kirim"
    )
